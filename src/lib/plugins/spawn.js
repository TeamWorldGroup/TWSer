...
module.exports.server = function(serv, options) {
  ...
  serv.destroyEntity = (entity) => {
    entity._writeOthersNearby('entity_destroy', {'entityIds': [entity.id]});
...
module.exports.player = function(player, serv, options) {
  player.commands.add({
    ...
    action(name) {
      ...
      if (entity.type == 'mob') serv.spawnMob(entity.id, player.world, player.position.scaled(1 / 32), {'velocity': Vec3((Math.random() - 0.5) * 10, Math.random() * 10 + 10, (Math.random() - 0.5) * 10)});
      else if (entity.type == 'object') serv.spawnObject(entity.id, player.world, player.position.scaled(1 / 32), {'velocity': Vec3((Math.random() - 0.5) * 10, Math.random() * 10 + 10, (Math.random() - 0.5) * 10)});
    }
  });
...
'name': args[1]
    ...
    action({number, name}) {
      ...
      for (let i = 0; i < number; i++) {
        if (entity.type == 'mob')
          serv.spawnMob(entity.id, player.world, player.position.scaled(1 / 32).offset(Math.floor(i / s * 10), 0, i % s * 10), {'velocity': Vec3((Math.random() - 0.5) * 10, Math.random() * 10 + 10, (Math.random() - 0.5) * 10)});
        else if (entity.type == 'object')
          serv.spawnObject(entity.id, player.world, player.position.scaled(1 / 32).offset(Math.floor(i / s * 10), 0, i % s * 10), {'velocity': Vec3((Math.random() - 0.5) * 10, Math.random() * 10 + 10, (Math.random() - 0.5) * 10)});
      }
    }
  });
