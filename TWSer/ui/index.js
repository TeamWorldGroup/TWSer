...
const windows = {
  ...
  'minecraft:villager': TradeWindow
};
...
Window.prototype.findInventoryItem = function (itemType, metadata, notFull) {
  ...
  const end = this.inventorySlotStart + INVENTORY_SLOT_COUNT;
...
Window.prototype.acceptClick = function (click) {
  ...
  const invSlotEnd = this.inventorySlotStart + INVENTORY_SLOT_COUNT;
...
Window.prototype.acceptSwapAreaRightClick = function (click) {
  ...
  const item = this.slots[click.slot];
...
Window.prototype.emptySlotCount = function () {
  ...
  for (let i = this.inventorySlotStart; i < end; ++i) {
...
function TradeWindow(id, title, slotCount) {
  Window.call(this, id, 7, title, slotCount);
}
util.inherits(TradeWindow, Window);

TradeWindow.prototype.inventorySlotStart = 3;

TradeWindow.prototype.acceptUniqueClick = function (click) {
  if (click.slot === 0 || click.slot === 1 || click.slot === 2) {
    this.acceptNonInventorySwapAreaClick(click);
  }
};