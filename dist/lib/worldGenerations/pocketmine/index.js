   ...
   this.populators.push(ores);*/
   	...
   	populator.populate(this.level, chunkX, chunkZ, this.random);
		...
		value: function generateKernel() {
			...
			var bellSize = 1 / this.SMOOTH_SIZE;
			var bellHeight = 2 * this.SMOOTH_SIZE;

			for (var _sx = -this.SMOOTH_SIZE; _sx <= this.SMOOTH_SIZE; ++_sx) {
				this.GAUSSIAN_KERNEL[_sx + this.SMOOTH_SIZE] = [];

				for (var _sz = -this.SMOOTH_SIZE; _sz <= this.SMOOTH_SIZE; ++_sz) {
					var bx = bellSize * _sx;
					var bz = bellSize * _sz;
					this.GAUSSIAN_KERNEL[_sx + this.SMOOTH_SIZE][_sz + this.SMOOTH_SIZE] = bellHeight * Math.exp(-(bx * bx + bz * bz) / 2);
				}
			}
		}
	}, {
...
//# sourceMappingURL=../../../maps/lib/worldGenerations/pocketmine/index.js.map
