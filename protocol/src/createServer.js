...
batchProto.addType('insideBatch', ['endOfArray', {'type': ['buffer', {'countType': 'varint'}]}]);
...
function createServer(options, encryption) {
	...
	server.on('connection', function(client) {
		...
		client.encryptionEnabled = Boolean(encryption);
		...
		client.on('batch', function (packets) {
			let buf = zlib.inflateSync(packets.payload);
			let pack = batchProto.parsePacketBuffer('insideBatch', buf).data;

			pack.forEach((packet) => client.readEncapsulatedPacket(Buffer.concat([new Buffer([0xfe]), packet])));
		});
		client.on('zlibd', function(packets) {
		...
		client.writeBatch = function (packets) {
			...
			client.writePacket('batch', {payload});
		};
